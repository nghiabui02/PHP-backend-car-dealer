...
class Common
{
    ...
    public static function getProductImageUrl(string $fileName, Bucket $bucket, array|UploadedFile|null $file): string
    {
    ...
    public static function getEmployeeImageUrl(string $fileName, Bucket $bucket, array|UploadedFile|null $file): string
    {
        $filePath = 'employees_images/' . $fileName;
        $bucket->upload(file_get_contents($file), [
            'name' => $filePath
        ]);
        $getImage = $bucket->object($filePath);
